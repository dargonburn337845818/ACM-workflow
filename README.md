...
- **随机推荐**：从 CF 题集中抽取区间内题目，换一题自动排除已推荐
...
- 自动翻译为中文，支持 **MyMemory / LibreTranslate / DeepSeek / 本地离线** 多后端
...
- 内置生成器：随机整数数组 / 树 / 图 / 字符串 / 排列，种子可复现
- 支持自定义脚本：`.js` / `.py` / `.cpp`，输出自动填充测试框
...
内置 8 大分类、47 个算法节点的中文算法手册，每个算法附 C++17 精简模板，可一键复制。

### 🎨 硬边墨色美化

一键应用 / 还原：活动栏沉底、单标签页、隐藏状态栏、硬边墨色配色。
...
tools/                        # 本地离线翻译：安装脚本 + 极简 Argos HTTP 服务
