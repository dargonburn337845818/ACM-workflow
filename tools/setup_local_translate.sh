...
#
# 功能：
#   1. 检查 $LLAMA_DIR 下 llama-server.exe 与 Hy-MT2 GGUF 是否存在
...
if [ ! -x "$LLAMA_SERVER" ]; then
  ...
  echo "请确认 $LLAMA_DIR 已解压 llama.cpp Windows 版本（支持 Q6_K 即可）。" >&2
  exit 1
fi
...
if [ ! -f "$LLAMA_MODEL" ]; then
  ...
  echo "请确认 $LLAMA_DIR 下存在 Hy-MT2-1.8B-Q6_K.gguf。" >&2
  exit 1
fi
...
echo '   "acmWorkflow.llamaDir": "<your-llama-dir>"'
echo
echo " 手动启动（仅确保服务）："
