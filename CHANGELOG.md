...
最新版本：**0.18.1**（WSL 适配）— 一键 WSL 环境脚本、Windows 路径自动转 `/mnt`、浏览器/编译/翻译适配。

完整迭代历史见 [docs/changelog.md](docs/changelog.md)：

- **0.18.1** WSL 适配：新增 `tools/setup_wsl.sh`；`baseDir`/`templatePath`/`dbPath`/`browserPath` 自动把 Windows 路径转 WSL `/mnt` 路径；`.py` 造数据脚本使用 `python3`；WSL 浏览器探测与 `--no-sandbox` 适配；本地离线翻译工具脚本；生成 `题面.md`；修复本地翻译公式占位符泄漏；修复 CF `$$$` 公式排版；记录/比赛直接使用 CF 登录态并移除手动绑定；文档补充。
