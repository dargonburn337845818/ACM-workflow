...
- **薄弱点推荐**：基于本地 AC 记录 × 题集标签，计算每个标签的提交/通过率，
  从通过率最低的 2~3 个专题中随机抽**未 AC** 的题 —— 按提交通过率补齐薄弱专题
...
- **一次创建**：生成 `Contest_{id}/contest_{id}_{index}.cpp` + 人读 .prob + CPH .cph 配置，
...
**按样例生成（原“AI 生成脚本”，V0.23 轻量化）**：造数据页顶部「按样例生成」按钮，自动读取当前题目 `.prob` 里的官方样例，**不调用 LLM**，直接按样例输入的行数、每行 token 数和类型生成随机化 Python 脚本；毫秒级完成，通过轻量验证后保存到当前题目目录 `gen.py`，并自动更新/追加流水线里的「自定义脚本」步骤。Spark 作为可选“理解题面”模式保留，默认不自动启动，且参数已降为轻量版（ctx 8192 / threads 8 / GPU 0 / 默认关闭）。
