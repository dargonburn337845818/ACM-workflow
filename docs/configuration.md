...
| `acmWorkflow.translateProvider` | enum | `"auto"` | 题面翻译后端：<br>• `auto` — MyMemory（国内可达、无需 key）+ Google 兜底，零配置<br>• `libre` — LibreTranslate，端点用 `libreEndpoint` 配置（可自建）<br>• `deepseek` — DeepSeek API，密钥存系统密钥链（见下） |
